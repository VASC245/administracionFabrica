<template>
  <div class="max-w-4xl mx-auto bg-white shadow-lg rounded-xl p-8 mt-10">
    <h2 class="text-3xl font-bold text-gray-800 mb-6 text-center">Registro de Fundas Plasticas</h2>
        ...
        <table class="min-w-full border border-gray-300 rounded-lg text-sm">
          ...
          <tbody>
            ...
            >
              ...
              <td class="px-4 py-2 border">{{ registro.fundas }} (Fundas)</td>
...
const filtrarPorFechas = async () => {
  let query = supabase.from('fundas_plasticas').select('*').order('fecha', { ascending: false })
...
const guardar = async () => {
  if (editMode.value) {
    ...
    const { error } = await supabase.from('fundas_plasticas').insert([nuevo.value])
    if (!error) {
      nuevo.value = { fecha: '', fundas: null }
      await cargar()
    }
...
const eliminarRegistro = async (id) => {
  const { error } = await supabase.from('fundas_plasticas').delete().eq('id', id)
